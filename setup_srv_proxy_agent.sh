podman run --name mysql -v $(pwd)/my.cnf:/etc/my.cnf:Z --network podman  -p 3306:33060 -d docker.io/mysql/mysql-server:latest
sleep 15
source $(pwd)/env.sh
...
podman run --name zabbix-server --network podman  -p 10051:10051 -e DB_SERVER_HOST="mysql.dns.podman" -e MYSQL_USER="zabbix" -e MYSQL_PASSWORD=$MYSQL_PASSWORD -d docker.io/zabbix/zabbix-server-mysql:latest

podman run --name zabbix-web -v $(pwd)/httpd.conf:/etc/apache2/httpd.conf:Z --network podman -e DB_SERVER_HOST="mysql.dns.podman" -e MYSQL_USER="zabbix" -e MYSQL_PASSWORD=$MYSQL_PASSWORD -e ZBX_SERVER_HOST="zabbix-server.dns.podman" -e PHP_TZ="Europe/Oslo" -p 8080:8080 -d docker.io/zabbix/zabbix-web-apache-mysql:latest
